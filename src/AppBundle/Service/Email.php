...
/**
 * Created by Vscode
 * User: HOUNSINOU HOUENOU Abed Nego Fifa
 * Date: 02/10/2022
 * Time: 22:21
 */
...
class Email
{
    ...
    public function sender($sujet, $emailDestinataires, $template, array $ccAdresses = [], $attachement = null, $from = 'Azr Exchange')
    {
        if (!$emailDestinataires) {
            return false;
        }
        if (!is_array($from)) {
            $from = ['abnefi.abnefi@gmail.com' => $from];
        }
